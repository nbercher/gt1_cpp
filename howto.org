...
#+title: G.Tech 1A -- Projet#7 -- C++ sous Linux / Git

* Mini "cheat sheet" Emacs (et Bash)
 ...
 Début sélection: C-Space     Copier sélection: M-x     Copier région: M-w

 Undo: M-x ENTER puis undo ENTER (normalement C-_, mais Windows gobbe ce raccourci!!)

 Aller au début ou fin de ligne: C-a ou C-e

 Rechercher: C-s

 Indenter le code: TAB

* Howto: Configuration de Git et du Git workflow (jeu Snake)
** Hello Git!
...
En résumé:
 ...
 - Si j'ai plusieurs machines pour travailler, j'utiliserai =git pull=
   ...
   (Diagrame simplifié: machine1 <--push/pull--> Github <--push/pull--> machine2)
